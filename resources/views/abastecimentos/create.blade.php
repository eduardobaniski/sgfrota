    ...
    <div class="px-4 sm:px-6 lg:px-8">
      <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
          <h1 class="text-base font-semibold leading-6 text-gray-900">Novo abastecimento</h1>
          <p class="mt-2 text-sm text-gray-700">Preencha os dados do abastecimento.</p>
        </div>
        <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
          <a href="{{ route('abastecimentos.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Cancelar</a>
        </div>
      </div>
    <form method="POST" action="{{ route('abastecimentos.store') }}" class="space-y-6">
      @csrf

      <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div>
          <label for="caminhao_id" class="block text-sm font-medium text-gray-700">Caminhão (ID)</label>
          <input type="number" id="caminhao_id" name="caminhao_id" value="{{ old('caminhao_id', $defaults['caminhao_id'] ?? '') }}" required
        ...
        <div>
          ...
          <input type="number" id="viagem_id" name="viagem_id" value="{{ old('viagem_id', $defaults['viagem_id'] ?? '') }}"
        ...
        <div>
          ...
          <input type="number" id="litros" name="litros" step="0.001" value="{{ old('litros') }}" required
        ...
        <div>
          ...
          <input type="number" id="preco_por_litro" name="preco_por_litro" step="0.001" value="{{ old('preco_por_litro') }}"
      ...
      <div>
        ...
        <textarea id="observacoes" name="observacoes" rows="4"
                  @class(['mt-1 block w-full p-2 border rounded-md shadow-sm', 'border-red-500' => $errors->has('observacoes'), 'border-gray-300' => ! $errors->has('observacoes')])>{{ old('observacoes') }}</textarea>
        @error('observacoes')
          <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        @enderror
      </div>
